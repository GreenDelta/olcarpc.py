...
class FlowPropertyTest(unittest.TestCase):
    ...
    def test_non_existing(self):
        # check for ID
        status = self.client.flow_property('non existing FlowProperty')
        self.assertFalse(status.ok)
        self.assertTrue(len(status.error) > 0)

        # check for name
        status = self.client.flow_property(name='non existing FlowProperty')
        self.assertFalse(status.ok)
        self.assertTrue(len(status.error) > 0)
    ...
    def test_get_flow_properties(self):
        ...
        for flow_property in self.client.flow_properties():
    ...
    def test_flow_property_atts(self):
        ...
        clone: rpc.FlowProperty = self.client.flow_property(orig.id).flow_property
